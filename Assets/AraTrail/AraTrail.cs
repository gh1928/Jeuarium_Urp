...
namespace Ara{
    ...
    public class AraTrail : MonoBehaviour {
        ...
        public void Awake()
        {
            ...
            if (target != null)
            {
                localMatrix = MatrixExtensions.GetLocalMatrix(transform, target);
            }
        }
        ...
        void OnEnable()
        {
            ...
            //points.Add(new Point( prevPosition, Vector3.zero, Vector3.forward, Vector3.up, color, thickness, 0,0));
            //points.Add(new Point(prevPosition, Vector3.zero, Vector3.forward, Vector3.up, color, thickness, 0, 0));
            //points.Add(new Point(prevPosition, Vector3.zero, Vector3.forward, Vector3.up, color, thickness, 0, 0));
        ...
        private void SyncOnTarget()
        {
            Matrix4x4 m = target.localToWorldMatrix * localMatrix;
            transform.position = MatrixExtensions.ExtractPosition(m);
            transform.rotation = MatrixExtensions.ExtractRotation(m);
        }
        ...
        public void EmitPoint(Vector3 position)
        {
            ...
            if (points.Count > 0)
            {
            //    float additional = Vector3.Distance(position, points[points.Count - 1].position);
            //    //currentDistance += additional;
            //    if (isUvDecent)
            //    {
            //        texcoord = points[points.Count - 1].texcoord - additional;
            //        if (texcoord < 0.1f)
            //            isUvDecent = false;
            //    }
            //    else
            ...
            //    }
                ...
                texcoord = //currentDistance; //
                                            points[points.Count - 1].texcoord + Vector3.Distance(position, points[points.Count - 1].position);
            }

            points.Add(new Point(position, initialVelocity + velocity * inertia, transform.right, transform.forward, initialColor, initialThickness, texcoord, 
                time*points.Count));
        }
        ...
        private void UpdateSegmentMesh(int start, int end, Vector3 localCamPosition)
        {
            ...
            if (trail.Count > 1)
            {
                ...
                float vCoord = textureMode == TextureMode.Stretch ? 0 : 0;//-uvFactor * totalLength * tileAnchor;
                ...
                for (int i = trail.Count - 1; i >= 0; --i)
                {
                    ...
                    float normalizedLength = sorting == TrailSorting.OlderOnTop ? partialLength / totalLength :
                        partialLength < 1 ? partialLength * 0.5f :
                        0.5f; //(totalLength - partialLength) / totalLength;
                    ...
                    float sectionThickness = thickness * data[i].thickness * thicknessOverTime.Evaluate(normalizedLife) *
                        thicknessOverLength.Evaluate(normalizedLength);
                    ...
                    vCoord += uvFactor * (sorting == TrailSorting.NewerOnTop ? uvDelta : -uvDelta);// *uvMultiply;
                }
            }
        }
        ...
        private void AppendSection(Point[] data, ref CurveFrame frame, int i, int count, float sectionThickness, float vCoord)
        {
            ...
            for (int j = 0; j <= sectionSegments; ++j)
            {
                ...
                uv.x = Map( (j / (float)sectionSegments) * uvWidthFactor, 0f, 1f, 0.1f, 0.9f);
                uv.y = vCoord;
                uv.z = 0;
                uv.w = 1;
                ...
                if(canvasCamera!=null)
                    uvs1.Add(canvasCamera.WorldToViewportPoint(vertex));
        ...
        private void AppendFlatTrail(Point[] data, ref CurveFrame frame, int i, int count, float sectionThickness, float vCoord, ref int va, ref int vb)
        {
            ...
            {
                uv.Set( Map(vCoord,0f,1f,0.1f,0.9f), sorting == TrailSorting. NewerOnTop ? uvWidthFactor : 0, 0, 1);
                uvs.Add(uv);
                uv.Set(Map(vCoord, 0f, 1f, 0.1f, 0.9f), sorting == TrailSorting.NewerOnTop ? 0 : uvWidthFactor, 0, 1);
                uvs.Add(uv);
            }
            ...
            if (hqCorners && cornerRoundness > 0)
            {
                ...
                for (int p = 0; p <= cornerRoundness; ++p)
                {
                    ...
                    tangents.Add(tangent);
                    vertColors.Add(color);
                    uv.Set(Map(vCoord, 0f, 1f, 0.1f, 0.9f), curvatureSign > 0 ? 0 : 1, 0, 1);
                    uvs.Add(uv);
...
{
    public static Matrix4x4 GetLocalMatrix(Transform transform, Transform target)
    {
        return transform.worldToLocalMatrix * target.localToWorldMatrix;
    }
